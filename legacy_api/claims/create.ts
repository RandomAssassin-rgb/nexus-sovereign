...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { worker_id, amount, status, type, reason, lat, jep_data } = payload;
    const lng = payload.lng || payload.lon;
    ...
    if (existingClaim) {
      return res.json({
        ...
        claim: existingClaim,
        replayed: true,
        message: 'Claim already exists. Returning existing ledger record.',
      });
    }
