...
import { EventTwinManager, EventTwin } from "./eventTwin.js";
...
export interface SimulationUser {
  ...
  declared_earnings?: number | string | null;
...
const DEMO_PARTICIPANTS: SimulationUser[] = [
...
type SimulationUserCacheState = {
  users: SimulationUser[];
  fetchedAt: number;
  pending: Promise<SimulationUser[]> | null;
};

interface SimulationSampleResult {
  partnerId: string;
  payout: number;
  persona: string;
  p_max: number;
...
export function getSimulationPartnerId(user: SimulationUser) {
  return String(user.partnerId || user.partner_id || "").trim();
}

export function getSimulationPreset(type: string): SimulationPreset {
...
export function selectSimulationUsers(users: SimulationUser[], type: string) {
...
export function countSimulationRecipients(users: SimulationUser[]) {
  return users.reduce((count, user, index, collection) => {
    const partnerId = getSimulationPartnerId(user);
    if (!partnerId) return count;
    const firstIndex = collection.findIndex(
      (candidate) => getSimulationPartnerId(candidate) === partnerId
    );
    return firstIndex === index ? count + 1 : count;
  }, 0);
}

export function buildSimulationAck(users: SimulationUser[], type: string, message?: string) {
  ...
  const sampleResults: SimulationSampleResult[] = impactedUsers.slice(0, 5).map((user) => {
  ...
  return {
    ...
    averagePayout:
      impactedUsers.length > 0 ? Math.round(projectedTotalPayout / Math.max(1, sampleResults.length || impactedUsers.length)) : 0,
    message:
      message ||
...
export function buildSimulationBroadcastPayload({
  type,
  message,
  ack,
  simulationId,
  ...
  type: string;
  message?: string;
  ack: ReturnType<typeof buildSimulationAck>;
  simulationId: string;
  twin?: Record<string, unknown> | null;
  popupDelayMs?: number;
}): SimulationBroadcastPayload {
...
function getSimulationUserCacheStore() {
  const globalState = globalThis as typeof globalThis & {
    __nexusSimulationUserCache?: SimulationUserCacheState;
  };
...
function normalizeCachedSimulationUser(
  userLike: SimulationUser | Record<string, unknown> | null | undefined
): SimulationUser | null {
  if (!userLike) return null;

  const partnerId = getSimulationPartnerId(userLike as SimulationUser);
  ...
  return {
    ...
    platform: String(
      (userLike as any).platform ??
        (userLike as any).specific_platform ??
        (userLike as any).signin_platform ??
        "Blinkit"
    ),
    balance: Number((userLike as any).balance || 0),
    declared_earnings: Number(
      (userLike as any).declared_earnings ?? (userLike as any).declaredEarnings ?? 0
    ),
    last_lat: (userLike as any).last_lat ?? null,
    last_lng: (userLike as any).last_lng ?? null,
    h3_cell: (userLike as any).h3_cell ?? null,
    status: (userLike as any).status ?? "active",
  };
}

export async function fetchSimulationUsers(supabaseClient: any) {
  const collected: SimulationUser[] = [];

  for (let pageStart = 0; ; pageStart += SIMULATION_PAGE_BATCH) {
    const pageResults = await Promise.all(
      Array.from({ length: SIMULATION_PAGE_BATCH }, (_, offset) => {
...
async function fetchAndCacheSimulationUsers(supabaseClient: any) {
...
async function fetchSimulationUsersWithRetry(
  supabaseClient: any,
  attempts: number = 3,
  retryDelayMs: number = 900
) {
  const store = getSimulationUserCacheStore();
...
export function upsertSimulationUserCacheEntry(
  userLike: SimulationUser | Record<string, unknown> | null | undefined
) {
  ...
  const existingIndex = nextUsers.findIndex(
    (candidate) => getSimulationPartnerId(candidate) === normalized.partnerId
  );
...
export async function getCachedSimulationUsers(supabaseClient: any, maxAgeMs = 60_000) {
  const store = getSimulationUserCacheStore();
  ...
  if (!store.pending) {
    const fetchWithResilience = async () => {
      try {
        ...
        const demoUsers: SimulationUser[] = Array.from({ length: 1250 }, (_, i) => ({
...
export function primeSimulationUsers(supabaseClient: any, maxAgeMs = 60_000) {
  return getCachedSimulationUsers(supabaseClient, maxAgeMs).catch((error) => {
    console.warn("[Simulation] Cache warm failed:", (error as any)?.message || error);
    const store = getSimulationUserCacheStore();
    return store.users;
  });
}

interface ExecuteSimulationPersistenceInput {
  users: SimulationUser[];
  type: string;
  message?: string;
  supabaseServer: any;
  ensureSkeletonUser: (partnerId: string, initialBalance?: number, fullName?: string | null) => Promise<any>;
  logPrefix?: string;
  twin?: any;
  broadcastPayload?: SimulationBroadcastPayload | null;
}

interface SimulationWorkItem {
  partnerId: string;
  priorBalance: number;
  nextBalance: number;
  payout: number;
...
interface SimulationSignalPayload {
  id: string;
  amount: number;
  type: string;
  reason: string;
...
type SimulationSignalEntry = {
  claimId: string;
  createdAt: number;
  payload: SimulationSignalPayload;
};
...
function getSimulationSignalStore() {
  const globalState = globalThis as typeof globalThis & {
    __nexusSimulationSignalStore?: Map<string, SimulationSignalEntry>;
  };

  if (!globalState.__nexusSimulationSignalStore) {
    globalState.__nexusSimulationSignalStore = new Map<string, SimulationSignalEntry>();
  }
...
function formatLedgerDateTime(iso: string) {
  try {
...
function buildSimulationWorkItems(
  users: SimulationUser[],
  type: string,
  message?: string,
  context?: {
  ...
  return users
    ...
    .filter(Boolean) as SimulationWorkItem[];
}

export function publishSimulationSignals(workItems: SimulationWorkItem[], supabaseServer?: any) {
  const store = getSimulationSignalStore();
  ...
  workItems.forEach((item) => {
    ...
    const jepData =
      (typeof item.claimRow.jep_data === "object" && item.claimRow.jep_data !== null
        ? item.claimRow.jep_data
        : {}) as Record<string, unknown>;
    const payload: SimulationSignalPayload = {
      id: item.claimId,
      amount: item.payout,
      type,
      reason: claimReason,
      ...
      popup_delay_ms:
        Number.isFinite(Number(jepData.popup_delay_ms)) ? Number(jepData.popup_delay_ms) : undefined,
...
export function readSimulationSignal(partnerId?: string | null, afterClaimId?: string | null) {
  cleanupSimulationSignals();
 ...
 */
export async function orchestrateFinalistSimulation({
  type,
  message,
  supabaseServer,
  activeWorkers,
  ...
  const fabric = SignalFabric.createDemoFabric(variant as any);
...
export async function executeSimulationPersistence({
  users,
  type,
  message,
  supabaseServer,
  ensureSkeletonUser: _ensureSkeletonUser,
  logPrefix = "[Simulation]",
  twin,
  broadcastPayload = null,
}: ExecuteSimulationPersistenceInput) {
  ...
  const insertedClaims = new Map<string, { claimDbId: string; claimPublicId: string }>();
