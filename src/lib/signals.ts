...
export type SignalType = 'weather' | 'aqi' | 'traffic' | 'identity' | 'activity' | 'device';
export type SignalProvenance = 'Live' | 'Fallback' | 'Simulation';
...
export interface EventSignal {
  ...
  source: string;
  type: SignalType;
  value: number; // 0-100 normalized intensity or health
  observedAt: string;
  metadata?: Record<string, any>;
  freshness: number; // 0-100 numeric
  confidence: number; // 0-100 numeric
  provenance: SignalProvenance;
  freshnessLabel: SignalFreshness;
}

export interface SignalFabricState {
  signals: EventSignal[];
...
export class SignalFabric {
  private signals: EventSignal[] = [];

  private enrichSignal(signal: Omit<EventSignal, 'freshness' | 'confidence' | 'provenance' | 'freshnessLabel'>): EventSignal {
    ...
    return {
      ...
      provenance: (signal as any).provenance || 'Live', // Default to Live if not specified
      freshnessLabel: freshness > 60 ? 'Fresh' : 'Stale'
    };
  }

  addSignal(signal: Omit<EventSignal, 'freshness' | 'confidence'>) {
    this.signals.push(this.enrichSignal(signal));
  }

  getSignals(): EventSignal[] {
   ...
   */
  getIntensity(type: SignalType): number {
   ...
   */
  calculateContradictionIndex(): number {
   ...
   */
  static createDemoFabric(variant: 'monsoon' | 'heatwave' | 'clear' = 'monsoon'): SignalFabric {
