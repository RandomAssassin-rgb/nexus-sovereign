import { SignalFabric, EventSignal } from './signals.js';
import { EventTwin } from './eventTwin.js';
import type { EvidenceForensicResult } from './evidenceForensics.js';

export type FraudDecision = 'auto-approve' | 'hold' | 'escalate';

export interface GpsSpoofAnalysis {
  is_gps_spoof_suspected: boolean;
  location_jump_violation: boolean;
  static_coordinates_anomaly: boolean;
  activity_location_mismatch: boolean;
  /** Human-readable explanation for demo/UI consumption */
  spoof_reasons: string[];
}

export interface FraudMatrix {
  ...
  reason_codes: string[];
  weakest_link?: { label: string; score: number };
  gps_spoof: GpsSpoofAnalysis;
  evidence_forensics?: EvidenceForensicResult;
}
...
export class FraudEngine {
  ...
  static evaluate(
    claim: {
      ...
      /** Optional consecutive location samples for movement analysis */
      locationSamples?: Array<{ lat: number; lng: number; ts: number }>;
    },
    fabric: SignalFabric,
    twin?: EventTwin,
    forensics?: EvidenceForensicResult
  ): FraudMatrix {
    ...
    if (samples.length >= 2) {
      for (let i = 1; i < samples.length; i++) {
        ...
        if (distKm > 5 && deltaMinutes < 2) {
          ...
          spoof_reasons.push(
            `Impossible jump detected: ${distKm.toFixed(1)} km in ${deltaMinutes.toFixed(1)} min`
          );
          break;
        }
      }
    ...
    if (samples.length >= 3) {
      const uniqueCoords = new Set(
        samples.map((s) => `${s.lat.toFixed(6)},${s.lng.toFixed(6)}`)
      );
      ...
      if (uniqueCoords.size === 1 || (uniqueCoords.size / samples.length < 0.6)) {
        ...
        spoof_reasons.push(
          'GPS readings show zero variance — natural device drift absent (Mock Detected)'
        );
      }
    }
    ...
    if (isInsideDisruptionZone && showsLowActivity) {
      ...
      spoof_reasons.push(
        'Location indicates disruption zone but activity logs show no matching work'
      );
    }

    const is_gps_spoof_suspected =
      location_jump_violation || static_coordinates_anomaly || activity_location_mismatch;

    const gpsSpoofResult: GpsSpoofAnalysis = {
    ...
    if (twin && twin.footprint) locationTrust = 95;
    ...
    if (activityMatch < 50) reason_codes.push('HOLD_ACTIVITY_UNCERTAIN');
    ...
    if (deviceTrust < 70) reason_codes.push('HOLD_DEVICE_INTEGRITY_LOW');
    ...
    if (state.contradictionIndex > 40) reason_codes.push('HOLD_BEHAVIORAL_ANOMALY');
    ...
    if (forensics) {
      ...
      if (forensics.duplicate_image_flag) {
        ...
        forensics.reason_codes.forEach(c => {
          if (!reason_codes.includes(c)) reason_codes.push(c);
        });
      }
    ...
    const weakest = scores.reduce((prev, curr) => {
      ...
      if (curr.isOptional && curr.score > 80) return prev;
    ...
    const verification_score = Math.floor(
      final_event_match_score * 0.25 +
    ...
    let decision: FraudDecision = 'auto-approve';
    ...
    if (is_gps_spoof_suspected || isWeakestLinkTriggered || verification_score < 50) {
      ...
      if (is_gps_spoof_suspected) reason_codes.push('GPS_SPOOF_SUSPECTED');
      if (isWeakestLinkTriggered)
        reason_codes.push(
          `ESCALATE_WEAKEST_LINK_${weakest.label.toUpperCase().replace(' ', '_')}`
        );
      ...
      if (reason_codes.length === 0) reason_codes.push('HOLD_MARGINAL_TRUST_THRESHOLD');
    }

    const primary_reason =
      reason_codes.length > 0 ? reason_codes[0] : 'AUTO_VERIFIED_FABRIC';
