...
// Supabase client is passed as parameter to functions needing DB access.
// This keeps the module decoupled from the server initialization order.
...
export interface EvidenceMetadata {
  ...
  reverse_geocode_result: ReverseGeocodeResult | null;
  event_twin_id: string | null;
}
...
export type SignalClassification = 'positive' | 'undetermined' | 'anomaly' | 'hard_contradiction';

export interface DuplicateDetail {
  is_exact_match: boolean;
  is_near_duplicate: boolean;
  matched_claim_ids: string[];
  matched_worker_ids: string[];
  is_cross_worker: boolean;
  classification: SignalClassification;
}

export interface TimestampDetail {
  upload_vs_event: 'within_window' | 'before_event' | 'after_event' | 'unknown';
  time_delta_minutes: number | null;
  exif_upload_gap_minutes: number | null;
  suspicious: boolean;
  classification: SignalClassification;
}
...
export interface EvidenceForensicResult {
  metadata: EvidenceMetadata;
  status: 'passed' | 'review' | 'rejected' | 'hold';
  duplicate_image_flag: boolean;
  evidence_geo_mismatch_flag: boolean;
  evidence_timestamp_mismatch_flag: boolean;
  duplicate_detail: DuplicateDetail | null;
  timestamp_detail: TimestampDetail | null;
  geo_detail: GeoDetail | null;
  reason_codes: string[];
  explanations: string[];
}
...
export function extractExifData(base64Data: string): Partial<EvidenceMetadata> {
  const result: Partial<EvidenceMetadata> = {
    exif_timestamp: null,
    exif_lat: null,
    exif_lng: null,
    exif_make: null,
...
function parseTiff(buffer: Buffer, tiffStart: number, result: any) {
  // Byte order
  const isLittleEndian = buffer.toString('utf8', tiffStart, tiffStart + 2) === 'II';
  const readUInt16 = isLittleEndian ? (off: number) => buffer.readUInt16LE(off) : (off: number) => buffer.readUInt16BE(off);
  const readUInt32 = isLittleEndian ? (off: number) => buffer.readUInt32LE(off) : (off: number) => buffer.readUInt32BE(off);
...
function processIfd(buffer: Buffer, tiffStart: number, ifdOffset: number, read16: any, read32: any, result: any, type: string) {
  ...
  for (let i = 0; i < numEntries; i++) {
    ...
    // Tags we care about
    if (tag === 0x010f) result.exif_make = readString(buffer, tiffStart, entryOffset, count, valueOffset);
    if (tag === 0x0110) result.exif_model = readString(buffer, tiffStart, entryOffset, count, valueOffset);
    if (tag === 0x8769) exifSubIfdOffset = valueOffset; // Exif SubIFD
    if (tag === 0x8825) gpsIfdOffset = valueOffset; // GPS IFD
    if (tag === 0x9003) result.exif_timestamp = readString(buffer, tiffStart, entryOffset, count, valueOffset)?.replace(/^(\d{4}):(\d{2}):(\d{2})/, '$1-$2-$3');

    // GPS tags
    if (type === 'gps') {
      if (tag === 1) result.gpsLatRef = buffer.toString('utf8', entryOffset + 8, entryOffset + 9);
      if (tag === 2) result.exif_lat = readRationalArray(buffer, tiffStart, valueOffset, count, read32);
      if (tag === 3) result.gpsLngRef = buffer.toString('utf8', entryOffset + 8, entryOffset + 9);
      if (tag === 4) result.exif_lng = readRationalArray(buffer, tiffStart, valueOffset, count, read32);
    }
  }

  if (exifSubIfdOffset) processIfd(buffer, tiffStart, tiffStart + exifSubIfdOffset, read16, read32, result, 'sub');
  if (gpsIfdOffset) processIfd(buffer, tiffStart, tiffStart + gpsIfdOffset, read16, read32, result, 'gps');
  ...
  if (result.exif_lat && result.gpsLatRef) {
    if (Array.isArray(result.exif_lat)) {
      ...
      if (result.gpsLatRef === 'S') result.exif_lat *= -1;
    }
  }
  if (result.exif_lng && result.gpsLngRef) {
    if (Array.isArray(result.exif_lng)) {
      ...
      if (result.gpsLngRef === 'W') result.exif_lng *= -1;
    }
  }
}

function readString(buffer: Buffer, tiffStart: number, entryOffset: number, count: number, valueOffset: number): string | null {
  if (count <= 4) return buffer.toString('utf8', entryOffset + 8, entryOffset + 8 + count).replace(/\0$/, '');
  return buffer.toString('utf8', tiffStart + valueOffset, tiffStart + valueOffset + count).replace(/\0$/, '');
}

function readRationalArray(buffer: Buffer, tiffStart: number, valueOffset: number, count: number, read32: any): number[] {
  const vals: number[] = [];
...
export function computeContentHash(base64Data: string): string {
  const buffer = Buffer.from(base64Data, 'base64');
...
export function computePerceptualHash(base64Data: string): string {
  const buffer = Buffer.from(base64Data, 'base64');
...
export async function checkDuplicates(
  ...
  try {
    ...
    for (const claim of matches) {
      ...
      const forensics = (jep as any)?.evidence_forensics;
      if (!forensics?.metadata) continue;
    ...
    const classification: SignalClassification = exactMatches.length > 0 ? 'hard_contradiction' : 'anomaly';
...
export function checkTimestampIntegrity(
  ...
  let upload_vs_event: TimestampDetail['upload_vs_event'] = 'unknown';
  let time_delta_minutes: number | null = null;
  ...
  if (eventStartTime && eventEndTime) {
    ...
    if (uploadMs < startMs) {
      ...
      if (time_delta_minutes > 30) suspicious = true;
      ...
      if (time_delta_minutes > 1440) suspicious = true;
    } else {
  ...
  let exif_upload_gap_minutes: number | null = null;
  if (exifTimestamp) {
    ...
    if (exif_upload_gap_minutes > 2880) suspicious = true;
  }

  // Classification logic
  let classification: SignalClassification = 'positive';
...
// ═══════════════════════════════════════════════════════
//  REVERSE GEOCODING (Provider-Abstracted)
// ═══════════════════════════════════════════════════════
...
const NominatimProvider: ReverseGeocodeProvider = {
  name: 'nominatim',
  async geocode(lat: number, lng: number): Promise<ReverseGeocodeResult> {
...
function getGeoProvider(): ReverseGeocodeProvider {
...
export function checkGeoContradiction(
  geocodeResult: ReverseGeocodeResult | null,
  eventZone: string | null,
  eventCenterLat: number | null,
  eventCenterLng: number | null,
  ...
  if (uploadLat && uploadLng && eventCenterLat && eventCenterLng) {
    detail.distance_km = Math.round(
      haversineKm(uploadLat, uploadLng, eventCenterLat, eventCenterLng) * 10
    ) / 10;
...
export async function analyze(params: AnalyzeParams & { supabase?: SupabaseClient }): Promise<EvidenceForensicResult> {
  ...
  const timestamp_detail = checkTimestampIntegrity(
    uploadTs,
    exifData.exif_timestamp || params.exifTimestamp || null,
    params.eventStartTime || null,
    params.eventEndTime || null
  );
  ...
  if (timestamp_detail.suspicious) {
    ...
    if (
      timestamp_detail.exif_upload_gap_minutes &&
  ...
  const geo_detail = checkGeoContradiction(
    reverse_geocode_result,
    params.eventZone || null,
    params.eventCenterLat || null,
    params.eventCenterLng || null,
    params.uploadLat,
    params.uploadLng
  );
  ...
  if (geo_detail.is_contradictory) {
    ...
    if (geo_detail.distance_km && geo_detail.distance_km > 25) {
      ...
      explanations.push(
        `Evidence uploaded ${geo_detail.distance_km} km from the event zone — outside the disruption boundary`
      );
    } else {
  ...
  let status: EvidenceForensicResult['status'] = 'passed';
