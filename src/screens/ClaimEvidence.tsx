...
import { ArrowLeft, Camera, AlertCircle, CheckCircle2, Scale, FileText, Download, Share2, Loader2, Zap } from "lucide-react";
import { useLocation, useNavigate, useParams } from "react-router-dom";
import { useState, useRef } from "react";
import { cn } from "../lib/utils";
import { toPng } from "html-to-image";
import jsPDF from "jspdf";
import { captureDeviceImage } from "../lib/deviceCapabilities";
...
export default function ClaimEvidence() {
  ...
  const handleUpload = () => {
    void (async () => {
      const capture = await captureDeviceImage("prompt");
      if (capture.dataUrl) {
        setEvidence(capture.dataUrl);
      }
    })();
  };

  const handleSubmit = async () => {
    // Navigate to the full Tier 3 Challenge screen with claim context
    navigate("/tier3-challenge", {
      state: {
        claimId,
        claimData: claimData,
        originalRejection: claimData,
      },
    });
  };
        ...
        <div className="pt-4 border-t border-border/50">
          ...
          <div className="bg-orange-500/5 p-3 rounded-xl space-y-2 border border-orange-500/20">
            <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-wider">
              ...
              <span className={cn((claimData.ai_probability ?? 0) > 30 ? "text-destructive" : "text-amber-500")}>
                {(claimData.ai_probability ?? 0) > 30 ? "Suspicious" : "Inconclusive"}
              </span>
            </div>
            <div className="text-xs text-muted-foreground leading-relaxed">
              AI forensic engine analyzed the evidence for metadata tampering and GAN-based generation. 
              AI Probability: <span className="font-bold">{claimData.ai_probability ?? 0}%</span>. 
              {(claimData.ai_probability ?? 0) > 30 ? " High likelihood of digital manipulation detected." : " Evidence is inconclusive for automated verification."}
              <br /><br />
  ...
  {!evidence ? (
    ...
    >
      ...
      <div className="text-center">
        <p className="font-semibold text-sm text-primary">Upload Evidence</p>
        <p className="text-xs text-muted-foreground mt-1 uppercase font-bold tracking-widest">Select from Gallery</p>
      </div>
    </button>
  ) : (
