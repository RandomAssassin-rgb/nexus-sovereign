 ...
 */
async function ensureSkeletonUser(partnerId: string, initialBalance: number = 0) {
  ...
  console.log(`[Auth] Creating skeleton user for ${partnerId}...`);
  // Note: aadhaar_number is included here as per the Master Stability Plan.
  // If the column is missing, this insert will fail until the schema is synced.
  const { data: newUser, error: createError } = await supabaseServer
    ...
    .insert([{ 
      ...
      platform: "Blinkit",
      aadhaar_number: `6372-${Math.floor(Math.random() * 9000 + 1000)}-${Math.floor(Math.random() * 9000 + 1000)}`,
  ...
  if (createError) {
     console.error(`[Auth] Failed to create skeleton user: ${createError.message}`);
     throw createError;
  }
  return newUser;
}
...
app.post("/api/auth/register-user", async (req, res) => {
  const { platform, method, partnerId, phone, biometric_verified, face_descriptor } = req.body;
  ...
  try {
    ...
    const { data, error } = await supabaseServer
      ...
      .upsert({
        ...
        auth_method: method,
        phone,
        biometric_status: biometric_verified ? 'verified' : 'pending',
    ...
    res.json({ success: true, message: "Registration cached locally (Supabase table missing/error)" });
  }
});
...
app.post("/api/premium/activate", async (req, res) => {
  const { partnerId, planType } = req.body;

  // Validate planType
  const validPlans = ["basic", "standard", "pro"];
  const tier = validPlans.includes(planType) ? planType : "basic";
  ...
  try {
    ...
    console.log(`[Premium] Activated plan "${tier}" for ${partnerId} until ${premiumUntil.toISOString()}`);
    res.json({
      success: true,
      planType: tier,
...
// Initialize OpenAI for OpenRouter insights
...
app.post("/api/ml/calculate-premium", async (req, res) => {
  try {
    ...
    const weeks_enrolled = req.body.weeks_enrolled || 8;
    const declared_earnings = req.body.declared_earnings || 650.0;
    const normalized_trust = trust_score > 1 ? trust_score / 1000 : trust_score;

    const payload = {
      weather_severity: weather_severity || 0.1,
      traffic_density: traffic_density || 0.2,
      aqi_severity: aqi_severity || 0.1,
      trust_score: normalized_trust,
      persona: persona || "Blinkit",
      declared_earnings: declared_earnings,
      zone_risk_tier: zoneRisk,
      weeks_enrolled: weeks_enrolled,
      trigger_type: trigger_type || "rain"
    };

    const response = await axios.post(`${PYTHON_ML_URL}/predict/premium`, payload, {
...
// --- Premium Coverage Enrollment ---
app.post("/api/premium/activate", async (req, res) => {
  const { partnerId, planType } = req.body;
  if (!partnerId || !planType) {
    return res.status(400).json({ error: "Missing partnerId or planType" });
  }
  ...
  try {
    ...
    const { error } = await supabaseServer
      .from('users')
      .update({ premium_until: nextWeek.toISOString() })
      .eq('id', partnerId);

    if (error) {
      console.error("Supabase updated failed for premium:", error);
      throw error;
    }

    res.json({ success: true, premium_until: nextWeek.toISOString(), planType });
  } catch (error) {
    console.error("Failed to activate premium:", error);
    res.status(500).json({ error: "Internal server error during premium activation" });
  }
});
...
app.get("/api/admin/stats", async (req, res) => {
  try {
    ...
    const platformFloat = 750000; // Standard platform liquidity float
    ...
    res.json({
      ...
      reservePool: totalBalance + platformFloat,
...
app.get("/api/actuarial/inputs", async (req, res) => {
  try {
    ...
    const totalBalance = (balances || []).reduce((acc, curr) => acc + (Number(curr.balance) || 0), 0);
    const platformFloat = 500000;

    res.json({
      b_res: totalBalance + platformFloat,
...
app.post("/api/admin/simulate", async (req, res) => {
  ...
  try {
    ...
    console.log(`[Simulation] Type: ${type} | Base Amount: ${amount}`);
    ...
    for (const [index, u] of users.entries()) {
      ...
      let userAmount = amount + Math.floor(Math.random() * 100) - 50; 
      if (userAmount < 50) userAmount = 50;
      ...
      const { data: claimData, error: claimErr } = await supabaseServer.from('claims').insert({
        ...
        jep_data: {
          ...
          partnerPlatform: u.platform || "Nexus Platform",
    ...
    res.json({ success: true, count: processedCount, averageAmount: amount });
  } catch (err: any) {
