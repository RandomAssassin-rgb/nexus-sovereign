...
export function normalizeTrustScore(value, fallback = 0.5) {
...
export function normalizePersonaGroup(input) {
...
export function normalizePersonaLabel(input) {
    const group = normalizePersonaGroup(input);
    if (group === "amazon_flipkart")
        return "Amazon / Flipkart";
    if (group === "swiggy_zomato")
        return "Swiggy / Zomato";
    return "Blinkit / Zepto";
}
export function normalizeRiskTier(input) {
...
export function inferSeason(input) {
    ...
    if (explicit.includes("monsoon") || explicit.includes("rain"))
        return "monsoon";
    if (explicit.includes("dry"))
        return "dry";
...
export function inferEarningsTier(input) {
    ...
    const declaredRaw = typeof input.declaredEarnings === "number"
...
export function calculateReservePool(totalBalance) {
...
export function calculatePmax(input) {
    const fallbackCalculatedPayout = Math.max(0, Number(input.wBase || 0)) * Math.max(0, Number(input.incomeLossPct || 0));
    const calculatedPayout = typeof input.calculatedPayout === "number" && Number.isFinite(input.calculatedPayout)
...
export function calculateCoverageCap(persona) {
    ...
    const cap = Math.max(...triggerKinds.map((triggerKind) => calculateZeroTouchPayout({
