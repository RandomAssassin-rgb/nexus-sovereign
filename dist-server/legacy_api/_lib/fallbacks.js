...
export async function fetchLiveTraffic(lat, lon) {
...
export function buildFallbackWeather(lat, lon) {
...
export function buildFallbackAqi(lat, lon) {
...
export function buildFallbackTraffic(lat, lon) {
...
export function buildRiskInsight(input) {
    ...
    const flags = [];
    if (weatherMain === 'Rain')
        flags.push('rain-loaded micro-climate');
    if (aqi >= 150)
        flags.push('pollution drag');
    if (jamFactor >= 6.5)
        flags.push('traffic drag');
    if (flags.length === 0) {
...
export function estimatePremium(input) {
    ...
    const blendedRisk = weatherSeverity * 0.4 +
        trafficDensity * 0.2 +
        aqiSeverity * 0.15 +
        zoneRisk * 0.25;
