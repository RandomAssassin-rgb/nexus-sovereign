...
import { calculatePmax, calculateReservePool, calculateWeeklyPremium, calculateZeroTouchPayout, clamp, inferEarningsTier, normalizePersonaLabel, } from "./actuarial.js";
import { supabaseServer } from "./supabase.js";
import { ensureSkeletonUser } from "./supabaseHelper.js";
const FORECAST_HOURS = [2, 6, 24];
...
const DEFAULT_PRODUCT_CONTROLS = {
...
function minutesSince(timestamp, fallbackMinutes = 12) {
    if (!timestamp)
        return fallbackMinutes;
    return Math.max(0, Math.round((Date.now() - timestamp) / 60000));
}
function freshnessState(minutes, watchAt = 6, staleAt = 18) {
    if (minutes >= staleAt)
        return "stale";
    if (minutes >= watchAt)
        return "watch";
...
function buildPhoneCandidates(identifier) {
...
function getProductControlsStore() {
...
export function saveProductControls(input) {
...
async function loadUser(partnerId) {
    const resolved = await resolveWorkerIdentity(partnerId);
    return resolved.user;
}
export async function resolveWorkerIdentity(partnerId) {
...
let __reserveContextCache = null;
...
async function deriveSignalSnapshot(lat, lon) {
    ...
    const weatherScore = weather.weather?.[0]?.main === "Rain"
        ? 0.88
        : tempC >= 37
            ? 0.62
...
function deriveLikelyTrigger(snapshot, claimVelocity) {
    const candidates = [
...
function buildHorizonForecast(input) {
    const earningsAtRisk = roundAmount(input.hourlyRate * input.hours * input.incomeLossPct * (0.7 + input.riskScore * 0.3));
...
export async function buildProtectionForecast(input) {
    ...
    const riskScore = clamp(snapshot.weatherScore * 0.38 +
        snapshot.aqiScore * 0.22 +
        snapshot.trafficScore * 0.18 +
        claimVelocity * 0.22, 0, 1);
    ...
    const horizons = FORECAST_HOURS.map((hours) => buildHorizonForecast({
        hours,
    ...
    const recommendation = likelyTrigger.trigger === "heavy_rain_flood"
    ...
    return {
        ...
        horizons,
        shield_mode: {
            headline: likelyTrigger.trigger === "heavy_rain_flood"
...
export function buildDeviceTrustReport(payload) {
    ...
    const flags = [];
    if (!nativeApp)
        flags.push("web-session-only");
    if (!biometricsAvailable)
        flags.push("biometrics-unavailable");
    if (!secureStorageReady)
        flags.push("secure-storage-missing");
    if (locationPermission !== "granted")
        flags.push("location-not-granted");
    if (faceMatchDrift >= 0.35)
        flags.push("face-match-drift");
    return {
        success: true,
...
function calculateProofOfHustle(claims, platform) {
    ...
    const lightRainClaims = claims.filter((c) => lightRainTriggers.some((t) => c.trigger_type?.toLowerCase().includes(t)) ||
        c.type?.toLowerCase().includes('rain'));
    const heavyRainClaims = claims.filter((c) => weatherTriggers.some((t) => c.trigger_type?.toLowerCase().includes(t)));
    ...
    if (totalClaimCount > 0) {
        ...
        if (recentClaimCount > 0) {
            const recentLightRain = recentClaims.filter((c) => lightRainTriggers.some((t) => c.trigger_type?.toLowerCase().includes(t))).length;
...
export async function buildTrustPassport(partnerId) {
    ...
    const verificationConfidence = clamp((user.avatar_url ? 0.32 : 0) +
        ...
        (user.phone ? 0.14 : 0) +
        (user.platform ? 0.12 : 0), 0, 1);
    const payoutReliability = clamp(approvedClaims.length / Math.max(1, approvedClaims.length + rejectedClaims.length), 0, 1);
    const platformConsistency = clamp(0.48 +
        (user.platform ? 0.18 : 0) +
        Math.min(approvedClaims.length, 5) * 0.05 +
        (transactions.some((tx) => tx.type === "credit") ? 0.08 : 0) -
        Math.min(rejectedClaims.length, 3) * 0.04, 0, 1);
    const anomalyFlags = [
    ...
    const tier = trustScore >= 0.82 && verificationConfidence >= 0.74
        ? "trusted"
        : trustScore >= 0.6
            ? "review"
            : "elevated-risk";
    ...
    const totalPaid = approvedClaims.reduce((sum, claim) => sum + Number(claim.payout_inr || 0), 0);
    return {
        success: true,
        ...
        overview: {
            ...
            trend: approvedClaims.length >= rejectedClaims.length
        ...
        platform_consistency: {
            ...
            posture: platformConsistency >= 0.8
        ...
        recommendation: anomalyFlags.length === 0
        ...
        ledger: claims.slice(0, 4).map((claim) => ({
...
export async function buildPayoutExplanation(claimId) {
    let claim = (await supabaseServer.from("claims").select("*").eq("claim_id_str", claimId).maybeSingle()).data || null;
    if (!claim) {
    ...
    const confidenceScore = Number(toNumber(claim.jep_data?.confidence_score ?? claim.jep_data?.confidence ?? 0.84, 0.84).toFixed(2));
    return {
        success: true,
        ...
        narrative: `Claim ${claim.claim_id_str || claim.id} moved through the ${claim.status || "processing"} path with ${Math.round(confidenceScore * 100)}% confidence and a reserve-aware final payout of Rs ${settledAmount || payoutQuote.final_payout}.`,
    };
}
export async function buildReserveProjection() {
    const { users, claims, reservePool, activeWorkers } = await loadReserveContext();
    const burnToday = claims.reduce((sum, claim) => sum + Number(claim.payout_inr || 0), 0);
...
export async function buildOperationalFreshness() {
    ...
    const users = usersRes.data || [];
    const credits = transactions.filter((tx) => String(tx.type || "").toLowerCase() === "credit");
    const openAlerts = alerts.filter((alert) => !["dismissed", "resolved", "blocked"].includes(String(alert.status || "").toLowerCase()));
    const approvedClaims = claims.filter((claim) => String(claim.status || "").toLowerCase() === "approved");
    const assistedClaims = claims.filter((claim) => ["processing", "pending", "review", "assisted", "needs_review"].includes(String(claim.status || "").toLowerCase()));
    const disputedClaims = claims.filter((claim) => ["disputed", "rejected", "escalated"].includes(String(claim.status || "").toLowerCase()));
    const telemetryWorkers = users.filter((user) => user.last_lat && user.last_lng);
    ...
    const telemetryFreshnessMinutes = telemetryCoverage >= 0.7 ? 2 : telemetryCoverage >= 0.45 ? 6 : telemetryCoverage >= 0.2 ? 11 : 19;
    ...
    const services = [
        {
            ...
            last_event: formatRelativeTime(new Date(Date.now() - claimsFreshnessMinutes * 60000).toISOString()),
        ...
        },
        {
            ...
            last_event: formatRelativeTime(new Date(Date.now() - payoutsFreshnessMinutes * 60000).toISOString()),
        ...
        },
        {
            ...
            last_event: formatRelativeTime(new Date(Date.now() - alertsFreshnessMinutes * 60000).toISOString()),
        ...
        },
        {
            ...
            last_event: formatRelativeTime(new Date(Date.now() - telemetryFreshnessMinutes * 60000).toISOString()),
    ...
    const payoutSuccessRate = clamp(credits.length / Math.max(1, credits.length + disputedClaims.length), 0, 1);
    const avgReleaseSeconds = roundAmount(clamp(68 + assistedClaims.length * 3 + openAlerts.length * 2 - approvedClaims.length * 0.4, 42, 184));
    const straightThroughPct = clamp(approvedClaims.length / Math.max(1, approvedClaims.length + assistedClaims.length + disputedClaims.length), 0, 1);
    const queues = [
        {
            id: "autonomous",
...
export async function buildPartnerAnalytics() {
    ...
    claims.forEach((claim) => {
        const worker = users.find((user) => user.partnerId === claim.worker_id);
...
export async function buildFraudMesh() {
    ...
    const correlatedClusters = Array.from(byType.entries())
        ...
        severity: bucket.severityScore >= 8 ? "critical" : bucket.severityScore >= 5 ? "high" : bucket.severityScore >= 3 ? "medium" : "low",
        action: type === "Impossible Velocity"
        ...
        .slice(0, 5);
    const watchlist = alerts.slice(0, 5).map((alert, index) => {
        const user = users.find((candidate) => candidate.partnerId === (alert.worker_id || alert.worker));
        ...
        return {
            ...
            posture: index < 2 ? "Escalate immediately" : index < 4 ? "Monitor for duplicate payout attempts" : "Keep under watch",
    ...
    return {
        success: true,
        summary: {
            investigations_open: alerts.filter((alert) => !["dismissed", "blocked"].includes(String(alert.status || "").toLowerCase())).length,
            correlated_clusters: correlatedClusters.length,
            impossible_travel: alerts.filter((alert) => String(alert.type).includes("Velocity")).length,
            face_drift_cases: alerts.filter((alert) => String(alert.type).includes("Biometric")).length,
            device_mismatch_cases: alerts.filter((alert) => String(alert.type).includes("Account")).length,
            duplicate_claim_pressure: alerts.filter((alert) => String(alert.type).includes("Duplicate")).length,
            payout_surface: approvedClaims,
            data_provenance: provenance,
        },
        clusters: correlatedClusters,
        watchlist,
        timeline: alerts.slice(0, 6).map((alert) => ({
            id: alert.id,
            type: alert.type,
...
export async function buildUserInbox(partnerId) {
    const resolved = await resolveWorkerIdentity(partnerId);
    ...
    const txRes = await supabaseServer
        ...
        .order("created_at", { ascending: false })
        .limit(5);
    const claimItems = (syncRes.data || []).map((claim) => ({
    ...
    const transactionItems = (txRes.data || []).slice(0, 3).map((tx) => ({
        id: `txn-${tx.id}`,
    ...
    const items = [
        {
            ...
            kind: "trigger",
            severity: forecast.severity === "elevated" ? "warning" : "info",
...
function mapClaimRowToLedgerClaim(claim) {
    if (!claim)
        return null;
    ...
    const isManual = String(claim.type || "").toLowerCase().includes("manual") ||
...
function mapTransactionRowToLedgerTransaction(transaction) {
    if (!transaction)
        return null;
...
function readSnapshotString(local, key) {
...
function readSnapshotNumber(local, key) {
...
function readSnapshotBoolean(local, key) {
    const value = readSnapshotString(local, key);
    if (!value)
        return null;
    return value === "true";
}
function readSnapshotJsonArray(local, key) {
...
function compactLocalState(local) {
    return Object.fromEntries(Object.entries(local).filter(([, value]) => typeof value === "string" && value.trim().length > 0));
}
export async function buildWorkerStateSnapshot(partnerId) {
    const resolved = await resolveWorkerIdentity(partnerId);
    const canonicalPartnerId = resolved.partnerId || normalizeIdentityValue(partnerId);
    if (!canonicalPartnerId) {
        return {
    ...
    const local = compactLocalState({
        ...
        nexus_last_lat: user?.last_lat !== undefined && user?.last_lat !== null ? String(user.last_lat) : null,
        nexus_last_lng: user?.last_lng !== undefined && user?.last_lng !== null ? String(user.last_lng) : null,
        nexus_wallet_balance: user?.balance !== undefined && user?.balance !== null ? String(user.balance) : null,
        ...
        nexus_premium_upgraded: user?.premium_upgraded !== undefined && user?.premium_upgraded !== null
...
export async function persistWorkerStateSnapshot(input) {
    const requestedPartnerId = normalizeIdentityValue(input.partnerId) || normalizeIdentityValue(input.snapshot?.partnerId);
    ...
    try {
        ...
        const { error: userError } = await supabaseServer
            ...
            .abortSignal(AbortSignal.timeout(15000));
        if (userError)
            console.warn("[Snapshot] User upsert warning:", userError.message);
        // 2. High-performance batch claims upsert
        const claims = readSnapshotJsonArray(local, "nexus_claims").slice(0, 30);
        let claimsAdded = 0;
        if (claims.length > 0) {
            ...
            const { error: claimErr } = await supabaseServer
                ...
                .abortSignal(AbortSignal.timeout(15000));
            if (!claimErr)
                claimsAdded = claimPayloads.length;
        }
        // 3. High-performance batch transactions upsert
        const txs = readSnapshotJsonArray(local, "nexus_transactions").slice(0, 30);
        let txsAdded = 0;
        if (txs.length > 0) {
...
export async function buildLatestPayoutSignal(input) {
    ...
    const isRecentSimulationClaim = Number.isFinite(claimTimestamp) && Date.now() - claimTimestamp <= 15 * 60 * 1000;
    ...
    const latestClaimId = claimRow && isRecentSimulationClaim ? String(claimRow.claim_id_str || claimRow.id || "").trim() : null;
...
export async function buildScenarioStudio(input) {
    ...
    return {
        ...
        p_max: reserve.p_max,
        runway_days_after_scenario: Number(((Math.max(0, reserve.reserve_pool - projectedLoad) || 1) / Math.max(1, reserve.burn_rate_per_day)).toFixed(1)),
        controls: {
            payout_corridor: productControls.payout_corridor,
            replacement_ratio: productControls.replacement_ratio,
            trigger_sensitivity: productControls.trigger_sensitivity === "expansive" && workersImpacted > reserve.active_workers * 0.18
                ? "standard"
        ...
        economics: {
            ...
            solvency_posture: projectedLoad > reserve.reserve_pool * 0.18
...
export async function buildAuditTrace() {
    ...
    const traces = [
        ...(claimsRes.data || []).map((claim) => ({
            ...
            severity: claim.status === "rejected" ? "warning" : claim.status === "approved" ? "success" : "info",
        })),
        ...(txRes.data || []).map((tx) => ({
            id: `txn-${tx.id}`,
        ...
        ...(alertsRes.data || []).map((alert) => ({
            ...
            severity: String(alert.severity || "").toLowerCase() === "critical"
                ? "critical"
