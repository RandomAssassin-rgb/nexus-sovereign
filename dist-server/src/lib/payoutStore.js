...
import { rehydratePlanFromServer } from "./premiumPlans";
 ...
 */
export function addClaimLocally(claim) {
    try {
 ...
 */
export async function saveClaims(claims, syncToServer = true) {
    ...
    if (syncToServer && claims.length > 0) {
        ...
        if (partnerId) {
            try {
                ...
                if (!response.ok)
                    throw new Error("Backend claim sync failed");
 ...
 */
export async function saveTransactions(txns, syncToServer = true) {
    ...
    if (syncToServer && txns.length > 0) {
        ...
        if (partnerId) {
            try {
                ...
                if (!response.ok)
                    throw new Error("Backend transaction sync failed");
 ...
 */
export async function setBalance(val, syncToServer = true) {
    ...
    if (syncToServer) {
        ...
        if (partnerId) {
            try {
                ...
                if (!response.ok)
                    throw new Error("Backend balance sync failed");
...
export function savePaymentMethods(methods, syncToServer = true) {
...
export async function pushPlanUpdate(partnerId, tier, until) {
    if (!partnerId)
        return;
...
export async function pushWalletUpdate(balance, transaction) {
    const partnerId = localStorage.getItem("partner_id");
    if (!partnerId)
        return;
...
export async function pushClaimCreate(claim) {
    const partnerId = localStorage.getItem("partner_id");
    if (!partnerId)
        return;
...
export function getPolicyStatus() {
    ...
    if (!until)
        return { isActive: isUpgraded, validTill: "N/A", daysLeft: 0, isUpgraded };
 ...
 */
export async function syncWithServer(partnerId, source = "poll") {
    if (!partnerId)
        return;
    ...
    try {
        ...
        const data = await parseJsonOrThrow(res, "Sync failed");
        ...
        if (data.user) {
            ...
            if (data.user.platform)
                localStorage.setItem("signin_platform", data.user.platform);
            if (data.user.phone)
                localStorage.setItem("signin_phone", data.user.phone);
            if (data.user.aadhaar_number)
                localStorage.setItem("nexus_aadhaar_number", data.user.aadhaar_number);
            ...
            rehydratePlanFromServer(data.user.premium_tier ?? null, data.user.premium_until ?? null);
            ...
            if (data.user.last_lat)
                localStorage.setItem("nexus_last_lat", data.user.last_lat);
            if (data.user.last_lng)
                localStorage.setItem("nexus_last_lng", data.user.last_lng);
        }
        if (data.transactions && data.transactions.length > 0) {
            const mappedTxns = data.transactions.map((t) => ({
                id: t.id,
                title: t.title,
                desc: t.description,
        ...
        if (data.claims && data.claims.length > 0) {
            ...
            const localClaims = existingRaw2 ? JSON.parse(existingRaw2) : [];
            const serverIds = new Set(mappedClaims.map((c) => c.id));
            // Only keep local claims that are NOT yet on the server (e.g. just submitted)
            const localOnlyClaims = localClaims.filter((c) => !serverIds.has(c.id) && !SEED_CLAIMS.some(s => s.id === c.id));
 ...
 */
export function generateZeroTouchPayout(lat, lng) {
    const now = new Date();
    ...
    const platformSource = localStorage.getItem("specific_platform") ||
...
export function getNotifications() {
    const claims = getClaims();
    const txns = getTransactions();
    const readIds = JSON.parse(localStorage.getItem(NOTIFICATIONS_READ_KEY) || "[]");
    const notifications = [];
 ...
 */
export function initRealtimeSubscription(partnerId) {
    if (!partnerId)
        return;
...
//# sourceMappingURL=payoutStore.js.map