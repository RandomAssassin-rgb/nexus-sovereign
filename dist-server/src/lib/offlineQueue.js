...
export async function saveOfflineClaim(claim) {
...
export async function removeOfflineClaim(claimId) {
...
async function syncOfflineClaimsInternal(targetIds) {
    const queue = await getOfflineClaims();
    ...
    for (let index = 0; index < nextQueue.length; index += 1) {
        ...
        if (!claim?.id)
            continue;
        if (targetSet && !targetSet.has(claim.id))
            continue;
        nextQueue[index] = {
            ...claim,
            status: "syncing",
...
export async function syncOfflineClaims() {
    return syncOfflineClaimsInternal();
}
export async function syncSingleOfflineClaim(claimId) {
