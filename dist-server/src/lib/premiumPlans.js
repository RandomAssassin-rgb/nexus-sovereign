...
export function getActivePlan(tier) {
    if (!tier || !(tier in PREMIUM_PLANS))
        return null;
    return PREMIUM_PLANS[tier];
}
/** Returns the next upgradeable tiers above the current one. */
export function getUpgradeTiers(currentTier) {
    if (!currentTier || !(currentTier in PREMIUM_PLANS))
...
export function getActiveStoredTier() {
    if (!isPremiumActive())
        return null;
...
export function savePlanToLocalStorage(planId, expiresAt) {
...
export function rehydratePlanFromServer(serverTier, serverUntil) {
    if (!serverTier || !serverUntil)
        return;
    const serverActive = new Date(serverUntil) > new Date();
    if (!serverActive)
        return; // expired on server — don't persist
    // Always trust server as source of truth
    savePlanToLocalStorage(serverTier, serverUntil);
}
//# sourceMappingURL=premiumPlans.js.map